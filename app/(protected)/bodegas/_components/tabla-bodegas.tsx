...
function MobileCard({ row, canEditar, isOwnConsejo }: { row: IBodegaResumen; canEditar: boolean; isOwnConsejo: boolean }) {
  return (
    <article className="border border-border rounded-lg p-4 space-y-3 bg-card">
      <header className="flex items-start justify-between gap-2">
        <div>
          ...
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mt-0.5">{row.nombre_consejo ?? row.tipo}</h3>
          {row.tipo === 'Consejo' && (
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[0.8125rem]">
        <div><span className="text-muted-foreground">Superficie: </span><span className="font-medium text-foreground">{row.superficie_m2 != null ? `${row.superficie_m2} m²` : '—'}</span></div>
        <div><span className="text-muted-foreground">Paquetes est.: </span><span className="font-medium text-foreground">{row.num_paquetes_estimados ?? '—'}</span></div>
      </div>
      <footer className="flex items-center justify-end gap-2 pt-2 border-t border-border">
        <Link href={`/bodegas/${row.id}`}>
          <Button variant="ghost" size="sm" className="gap-1.5 text-primary hover:text-primary/80" aria-label={`Ver bodega ${row.id}`}>
            <Eye className="h-4 w-4" aria-hidden="true" /><span>Ver</span>
          </Button>
        </Link>
        {canEditar && isOwnConsejo && (
    ...
    <div className="space-y-3" aria-hidden="true">
      {Array.from({ length: 6 }, (_, i) => (
        <div key={i} className="border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2"><Skeleton className="h-4 w-full animate-pulse motion-reduce:animate-none" /><Skeleton className="h-4 w-full animate-pulse motion-reduce:animate-none" /></div>
          <div className="flex justify-end pt-2 border-t border-border"><Skeleton className="h-8 w-16 rounded-md animate-pulse motion-reduce:animate-none" /></div>
        </div>
      ))}
    </div>
  );
...
export function TablaBodegas({ data, isLoading, isError, onRetry, tipo, tipoConsejo }: TablaBodegasProps) {
  ...
  async function handleExportar() {
    ...
    try {
      ...
      const match = cd?.match(/filename[^;=\s]*=((['"]).*?\2|[^;\n]*)/);
  ...
  const columns = useMemo<ColumnDef<IBodegaResumen>[]>(
    () => [
      ...
      },
      {
        id: 'actions',
        header: '',
        size: canEditar || canEliminar ? 110 : 80,
        enableSorting: false,
        enableHiding: false,
        cell: ({ row }) => {
          const esValidada = row.original.status === 'Validada';
          ...
          return (
            <div className="flex items-center justify-end gap-1">
              <Link href={`/bodegas/${row.original.id}`}>
                <Button variant="ghost" size="sm" className="gap-1.5 text-primary hover:text-primary/80" aria-label={`Ver bodega ${row.original.id}`}>
                  <span>Ver</span><Eye className="h-4 w-4" aria-hidden="true" />
                </Button>
              </Link>
              {canEditar && !esValidada && isOwnConsejo && (
              ...
              {canEliminar && !esValidada && isOwnConsejo && (
                <Button
                  variant="outline"
                  size="icon"
