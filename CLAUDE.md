...
- Patrón: `app/(protected)/<modulo>/` con `page.tsx` (server) + `_components/` + `_hooks/`. Los datos se leen con hooks de TanStack Query que exportan sus **query keys**; las mutaciones invalidan keys y avisan con `toastSuccess`/`toastError`. Tipos por dominio en `types/` (`IBodega`, `TComponenteFoto`).
- **Toda llamada al API** pasa por `apiClient` / `authClient` usando rutas de `lib/api/endpoints.ts` — nunca URLs literales ni `fetch` suelto. El interceptor ya desenvuelve el sobre `{ status, message, data }` del API y añade CSRF; la sesión vive en cookies HttpOnly, nunca en `localStorage`.
